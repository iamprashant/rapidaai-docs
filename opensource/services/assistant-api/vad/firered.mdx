...
5. Speech onset is confirmed only after the configured `start_secs` duration above the confidence threshold — this filters out short noise bursts
6. Same packet emission: `InterruptionPacket` start/end events on confirmed speech onset and offset
...
|-----------|---------|-------|-------------|
| `microphone.vad.confidence` | `0.7` | 0–1 | Speech probability threshold applied after smoothing |
...
Internally, FireRed converts `start_secs` and `stop_secs` into its postprocessor frame counts. Each FireRed postprocessor frame represents 10 ms.
...
| SpeechThreshold | 0.7 | Default smoothed probability threshold used when `microphone.vad.confidence` is unset |
