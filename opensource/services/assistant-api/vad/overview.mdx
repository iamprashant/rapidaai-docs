...
Voice Activity Detection (VAD) runs on every audio frame before STT. It determines whether the caller is speaking and emits interruption start/end signals for barge-in and turn lifecycle handling.
...
| `microphone.vad.confidence` | float | `0.7` | Speech probability threshold. Lower = more sensitive. |
| `microphone.vad.start_secs` | float | `0.2` | Continuous speech duration before confirming speech start. |
| `microphone.vad.stop_secs` | float | `0.2` | Continuous non-speech duration before confirming speech end. |

`start_secs` and `stop_secs` are public duration values. Each provider converts them to its native VAD frame count internally.
