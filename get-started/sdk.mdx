...
<CodeGroup>
  ```bash npm
  npm install rapida-react
  ```

```bash yarn
yarn add rapida-react
```

```bash pnpm
pnpm add rapida-react
```
...
<CodeGroup>
  ```bash npm
  npm install rapida-nodejs
  ```

```bash yarn
yarn add rapida-nodejs
```

```bash pnpm
pnpm add rapida-nodejs
```
...
<CodeGroup>
  ```bash pip
  pip install rapida-sdk
  ```

```bash pip3
pip3 install rapida-sdk
```
...
### Administration (Personal Token)

Use this for APIs that require write/admin access.
...
### Project Credentials (SDK API Key)
