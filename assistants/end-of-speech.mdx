...
| **Output** | Speech onset/offset events | End-of-speech signal that triggers LLM response |
| **Affected by** | Audio signal quality, background noise | Pause patterns, speech content, conversation context |

VAD sends speech start and end events. EOS uses VAD start to block premature completion while speech is active, and VAD end to flush a pending final transcript when one exists. Interim STT resets the EOS timer while more transcript text is still arriving.
