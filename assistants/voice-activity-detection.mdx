...
|-----------|-----------|---------|-------|-------------|
| Confidence | `microphone.vad.confidence` | `0.7` | 0.05 – 0.95 | Speech probability threshold. Each VAD frame produces a probability score between 0 and 1. Frames at or above this value are classified as speech. |
| Start Seconds | `microphone.vad.start_secs` | `0.2` | 0 – 2 | Continuous speech duration before confirming speech onset and triggering an interruption. |
| Stop Seconds | `microphone.vad.stop_secs` | `0.2` | 0 – 2 | Continuous non-speech duration before ending the active speech segment. |

<Tip>
**Confidence tuning guide:**
- **0.3 – 0.4** — Very sensitive. Use only in extremely quiet, controlled environments. Will pick up breathing, lip smacks, and faint background sounds.
- **0.5 – 0.6** — Balanced for clean audio.
- **0.7 – 0.8** — Strict. This is the default range and reduces false triggers in typical phone and web audio.
- **0.9 – 0.95** — Very strict. Only loud, clear speech triggers detection. Risk of missing normal conversational speech.
</Tip>
...
| Parameter | Config Key | Default | Range | Description |
|-----------|-----------|---------|-------|-------------|
| Confidence | `microphone.vad.confidence` | `0.7` | 0 – 1 | Speech probability threshold. Same semantics as Silero — frames at or above this value are classified as speech. |
| Start Seconds | `microphone.vad.start_secs` | `0.2` | 0 – 2 | Continuous speech duration before confirming speech onset. |
| Stop Seconds | `microphone.vad.stop_secs` | `0.2` | 0 – 2 | Continuous non-speech duration before ending a speech segment. |
...
| Parameter | Config Key | Default | Range | Description |
|-----------|-----------|---------|-------|-------------|
| Confidence | `microphone.vad.confidence` | `0.7` | 0 – 1 | Probability threshold for speech detection. Applied after smoothing — the postprocessor uses a moving average window (size 5) to smooth raw probabilities before comparing against this value. |
| Start Seconds | `microphone.vad.start_secs` | `0.2` | 0 – 2 | Continuous speech duration before confirming speech onset. Internally converted to FireRed's 10 ms postprocessor frames. |
| Stop Seconds | `microphone.vad.stop_secs` | `0.2` | 0 – 2 | Continuous non-speech duration before ending a speech segment. Internally converted to FireRed's 10 ms postprocessor frames. |

<Info>
FireRed VAD's postprocessor pads speech start by 5 frames (50 ms) to capture the onset of the utterance that triggered detection. The max speech frame limit is 2000 frames (20 seconds) — after which a forced speech end is emitted.
</Info>
...
### Start Seconds — barge-in sensitivity

The `start_secs` parameter controls how quickly the assistant detects that a caller has started speaking and triggers a **barge-in** (interrupting assistant speech).

- **Lower values (0.05–0.1 s):** The assistant stops speaking quickly when the caller makes sound. Good for fast-paced IVR-style interactions. Risk: short sounds like coughs or "um" trigger interruption.
- **Default (0.2 s):** Requires about 200 ms of sustained speech before interrupting the assistant. Filters out most accidental sounds.
- **Higher values (0.3–0.5 s):** The assistant only stops when the caller has clearly started a full utterance. Good for avoiding false interruptions during one-way information delivery. Risk: the caller has to speak longer before the assistant stops.

### Stop Seconds — speech segment boundary

The `stop_secs` parameter controls how long the VAD waits in silence before considering a speech segment finished.

- **Lower values (0.05–0.1 s):** Speech segments are split at short pauses. Words spoken with natural gaps between them may be split into multiple segments.
- **Default (0.2 s):** Covers typical intra-sentence pauses. Most natural speech is captured as a single continuous segment.
- **Higher values (0.3–0.5 s):** Only longer pauses end a segment. Good for speakers who pause frequently mid-sentence.

<Warning>
VAD stop seconds are different from End of Speech timeout. VAD determines **speech segment boundaries** (is the caller actively making sound right now?). End of Speech determines **turn boundaries** (has the caller finished their complete thought?). Both work together — VAD feeds activity signals to the EOS detector.
</Warning>
