...
| EOS | Better VAD start/end signals make turn detection more predictable. |
...
|---------|----------------|
| Background noise interrupts assistant speech | Keep RNNoise enabled and raise VAD confidence. |
| Quiet speakers are missed | Keep RNNoise enabled, then lower VAD confidence slightly. |
| Words are clipped at the start | Lower VAD confidence or reduce VAD start seconds. |
| Speech gets split into multiple chunks | Increase VAD stop seconds. |

See [Voice Activity Detection](/assistants/voice-activity-detection) for VAD provider and confidence/start/stop tuning details.
...
|---------|--------------|------------|
| Transcript includes background words or sounds | Noise cancellation off or VAD too sensitive | Enable RNNoise and raise VAD confidence. |
...
| Assistant still interrupts on noise | VAD confidence too low | Raise VAD confidence after enabling RNNoise. |
